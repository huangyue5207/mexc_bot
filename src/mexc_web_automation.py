...
class MexcWebAutomation:
    ...
    def handle_route(self, route, request):
        url = request.url
        if "8544-1417b79269c26f95.js" in url:
            logger.info(f"拦截到请求: {url}")
            # 读取本地修改过的 JS 文件
            with open("/Users/huangyue/Workspace/mexc_bot/js/static.mocortech.com/futures-v3/_next/static/chunks/8544-1417b79269c26f95.js", 
    ...
    def get_open_position(self) -> bool:
        """
        获取持仓
        """
        js_code = """() => {
    ...
    def get_open_stop_orders(self) -> bool:
        """
        获取止损订单
        """
        js_code = """() => {
    ...
    def long_entry(self) -> bool:
        ...
        多头开仓
        """
        self.open_position(self.pair, 1, 1, 5, self.amount, self.leverage)
        return True
    ...
    def short_entry(self) -> bool:
        ...
        空头开仓
        """
        self.open_position(self.pair, 3, 1, 5, self.amount, self.leverage)
        return True
    ...
    def close_open_position(self) -> bool:
        ...
        if ret and ret.get("data"):
            for position in ret["data"]:
                ...
                self.close_position(self.pair, position_type, 1, 5, position["positionId"], self.amount, self.leverage)
            return True
        return False
    
    def set_open_stop_loss(self, price: float, position_id: int) -> bool:
        """
        设置止损
        """
        ret = self.get_open_position()
        if ret and ret.get("data"):
            ...
            self.set_stop_loss(price, 1, position_id, "SAME", 2, 2)
            return True
        return False

    def cancel_short_stop_loss(self) -> bool:
        """
        取消止损
        """
        js_code = """() => {
    ...
    def set_stop_loss(self, price: float, loss_trend: int, position_id: int, profit_loss_vol_type: int, vol_type: int, take_profit_reverse: int) -> bool:
        """
        设置止损
        """
